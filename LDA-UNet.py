#Lightweight: shallow network, channel count: 32, 64, 128, 256, using depthwise separable convolution
#Downsampling and upsampling dilated convolution symmetry [1,2,4,2,1]
#The attention mechanism CBAM is explicitly added after each convolutional block in the encoding stage, and the attention output is fused through addition operation.
import torch
...
#After combining the network model, the code is:
#Depthwise Separable Convolution+Hollow Convolution
    ...
    def __init__(self, in_ch, out_ch, kernel_size=3, padding=1, dilation=1,channels_per_seg=128):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=in_ch, out_channels=in_ch, kernel_size=kernel_size, padding=padding,dilation=dilation,
                      groups=in_ch // C),
            nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=1, padding=0, dilation=1,groups=1)# 1*1 kernel does not use dilated convolution
        )
...
class conv_block(nn.Module):
    ...
    def __init__(self, ch_in, ch_out,dilation=1, mid_channels=None): #Default Void Rate: 1
        super().__init__()
        if not mid_channels:
            mid_channels = ch_out
...
class U_Net_CBAM(nn.Module):
    def __init__(self, img_ch, output_ch):
        ...
        #Downsampling
        self.Conv1 = conv_block(ch_in=img_ch, ch_out=32)  # 32
    ...
    def forward(self, x):
        # encoding path
        x1 = self.Conv1(x)
        x1 = self.cbam1(x1) + x1 #After CBAM, it is added and fused with x1
